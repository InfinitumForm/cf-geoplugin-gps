...
With the help of this plugin, Geo Controller users can use GPS to follow visitors. By utilizing this plugin, you can resolve the issue of correctly locating mobile visitors and fixing their location errors.

= INSTRUCTIONS =

Installing **[Geo Controller](https://wordpress.org/plugins/cf-geoplugin/)** and activating a legitimate Personal, Freelance, or Business license are prerequisites for using this plugin. Additionally, you must have the [Geocoding API](https://developers.google.com/maps/documentation/javascript/geocoding) enabled and a valid [Google Map API Key](https://developers.google.com/maps/documentation/javascript/get-api-key) in order to use the Google Map functionality.

When these conditions are satisfied, you can install and activate the plugin.

The fantastic news is that this plugin runs automatically and doesn't need any additional configuration.

To deliver the most precise GPS data, the plugin makes use of the Geo Controller API and GoogleGeocoding API services. So, to ensure a smooth experience, be sure to adhere to the above instructions.

Simple, right?

== Installation ==

Installing Geo Controller GPS extension is a breeze. Just follow these simple steps:

* Upload the plugin files to the **/wp-content/plugins/cf-geoplugin-gps** directory, or install the plugin through the WordPress plugins screen directly.
